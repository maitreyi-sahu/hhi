...
value_stamp <- "26Nov2023_0620_treatment_spline"

if(interactive()){
  stamp <- '7apr2024_hospital_hhi'
  cov <- cov
  #sim_path <- paste0(sims_path, "simbetas_", stamp)
  note <- "2000_2020"
  reg_type <- "timeFE"
  var <- ""
  ...
  ITERATOR <- fread(paste0(sim_path, "/iterator.csv"))
  cov <- ITERATOR[j, cov]
}
...
## Formatting
## ------------------------
if(reg_type %in% c("timeFE")){
  IDcols <- c("year_id", "location_name")
}else{
  IDcols <- c("location_name", "year_id")
}

# IMPORT Value, default when var is empty 
if(var == ""){
  
  value <- fread(paste0(usval_path,"sfa_output/sensitivity/", value_stamp, "/value_scores.csv")) %>%
    arrange(location_name,year_id, draw) %>%
    mutate(draw=paste0("d",draw+1)) %>%
    dcast(location_name+year_id~draw,value.var="value_score")
  
  
}else{
  
  # load in price data as dep var
  value <- fread(paste0(usval_path, "/shapley_decomp/", value_stamp, "/price_pca_data.csv"))
  
  # comfirm dep var is in the data
  if(!var %in% colnames(value)){
    stop(paste0("dependent var specified (", var, ") is not in price_pca_data.csv!"))
  }
...
if(note %like% "[0-9]{4}_[0-9]{4}"){
  
  start_year <- as.numeric(gsub("_[0-9]{4}", "", note))
  end_year <- as.numeric(gsub("[0-9]{4}_", "", note))
  print(paste0("Subsetting data to ", start_year, ":", end_year))
  
  value <- value %>% filter(year_id %in% start_year:end_year)
}

# determine number of draws
n_draws <- length(colnames(value)[grepl("^d", colnames(value))])
