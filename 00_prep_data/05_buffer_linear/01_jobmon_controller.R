...
# Controller script to launch the linear HHI's
# Output file: "linear_hhi_compiled_2024-04-03.feather" 
...
for (i in 1:nrow(combinations)) {
  ...
  task <- jobmonr::task(
    ...
    r_shell = r_shell,
    scriptname = paste0(code_path, "05_buffer_linear/02_linear_distance_hhi.R"),
...
compile_hhi <- function(radius) {
  x <- open_dataset(paste0(out_dir, "buffer/linear/radius=", radius), format = "feather") %>% collect()
  return(x)
}
...
# Save
stamp <- Sys.Date()
write_feather(compiled_hhi, paste0(out_dir, "buffer/linear/linear_hhi_compiled_", stamp, ".feather"))
